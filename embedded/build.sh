...
if [ ! -e $BUILDROOT/.config ]; then
    make -C $BUILDROOT BR2_EXTERNAL="$PWD/gitscan" gitscan_defconfig
fi

#
# Build everything
#
make -C $BUILDROOT BR2_EXTERNAL="$PWD/gitscan"

#
# Copy the files we are interested in from buildroot's "output/gitscan" directory
# to our "output" directory in top level directory 
#

# Copy Linux kernel and initramfs
cp $BUILDROOT/output/gitscan/rootfs.cpio.zst $BUILDROOT/output/gitscan/zImage output
# Raspberry Pi firmware files
cp $BUILDROOT/output/gitscan/rpi-firmware/start4.elf output
cp $BUILDROOT/output/gitscan/rpi-firmware/fixup4.dat output
cp $BUILDROOT/output/gitscan/*.dtb output
